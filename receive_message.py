...
def main():
    ...
    def callback_dpu(ch, method, properties, body):
        ...
        # gedi_dataset_ETL(message['url'])  # SR - uncomment this for actual use case
        print(" [x] Done")
    ...
    def callback_aoi(ch, method, properties, body):
        ...
        #     links.append(get_gedi_download_links(product, message['version'], message['bbox']))
        array_of_url_dicts.append(gedi02_A_dict) # SR - using sample data since above function call not working on my computer

        for url_dict in array_of_url_dicts:
            product_key = list(url_dict.keys())[0]
            for url in url_dict[product_key]:
                message = {'url': url}
                [channel, connection] = connect_to_rabbitmq(rab_config)
                produce_rabbitmq_message(channel, connection, 'download_parse_upload', message)
